#!/usr/bin/env python3
"""
Build Macro Static Weights
===========================

Calculates optimal STATIC (fixed) weights for the three-sleeve portfolio
across all macro states (NORMAL/CHOP/CRISIS).

This establishes the baseline weights that will be used by Path B to calculate
adjustment multipliers for CHOP and CRISIS states.
...
def load_sleeve_returns(sleeve_paths, start_date=None, end_date=None):
    ...
    for name, path in sleeve_paths.items():
        ...
        # Use pnl_net for strategy returns
        if 'pnl_net' in df.columns:
            ...
            raise ValueError(f"Sleeve {name} missing pnl_net or portfolio_ret column")
...
def calculate_max_sharpe(returns_df):
    ...
    returns_pct = returns / starting_capital
    
    # Mean returns and covariance
    mean_returns = returns_pct.mean(axis=0) * 252  # Annualized
    cov_matrix = np.cov(returns_pct.T) * 252  # Annualized
...
def calculate_grid_search(returns_df, step=0.05):
    ...
    for w1 in weight_range:
        for w2 in weight_range:
            ...
            portfolio_rets = returns_pct @ weights
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate optimal static weights for macro-static baseline"
    )
    ...
    parser.add_argument(
        ...
        help='Output directory for macro_static_weights.csv'
    )
    ...
    # Print header
    print("\n" + "=" * 80)
    print("MACRO STATIC WEIGHTS BUILDER")
    print("=" * 80)
    ...
    try:
        ...
        print(f"\n  Optimal Static Weights (IS {args.start_date[:4]}-{args.end_date[:4]}):")
        ...
        weights_df.to_csv(output_dir / 'macro_static_weights.csv', index=False)
        print(f"  ✓ Saved macro_static_weights.csv")
        ...
        metadata = {
            ...
            'note': 'These are baseline static weights. Path B will calculate adjustment multipliers for CHOP/CRISIS states.'
        }
        
        with open(output_dir / 'macro_static_weights_metadata.json', 'w') as f:
            json.dump(metadata, f, indent=2)
        print(f"  ✓ Saved macro_static_weights_metadata.json")
        
        # Print summary
        print("\n" + "=" * 80)
        print("MACRO STATIC WEIGHTS CALCULATION COMPLETE")
        print("=" * 80)
        print(f"Baseline weights: TM={weights['TrendMedium']:.1%}, "
              f"TI={weights['TrendImpulse']:.1%}, "
              f"MC={weights['MomentumCore']:.1%}")
        print(f"Baseline Sharpe:  {metrics['sharpe']:.4f}")
        print(f"\nWeights saved to: {output_dir / 'macro_static_weights.csv'}")
        print(f"\nNext step: Run Path B optimizer to calculate CHOP/CRISIS multipliers")
        print("=" * 80)
