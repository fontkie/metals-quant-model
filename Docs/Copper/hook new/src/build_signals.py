...
def vol_target_weights(raw_ret: pd.Series, target_vol=DEFAULT_VOL_TARGET, lb=DEFAULT_VOL_LB, max_lev=DEFAULT_LEV_CAP):
...
def run_strategy(df: pd.DataFrame,
                 low_th: float, high_th: float, volratio_cap: float,
                 cadence: str, exec_mode: str, hold_bars: int,
                 vol_target: float, vol_lb: int, lev_cap: float, cost_per_abs_delta: float):
    idx = df.index
    ret = df["ret"]
    price = df["Price"]
    ...
    def seg(mask: pd.Series):
        r = strat_ret[mask]
        if r.empty:
            return dict(sharpe=0.0, dd=np.nan, turnover=np.nan, trades=0, avg_hold=np.nan, pct_days=np.nan, hit=np.nan, ann_return=np.nan)
        ec = equity_curve(r)
        dd = max_drawdown(ec)
        ...
        return dict(sharpe=sharpe, dd=dd, turnover=turnover, trades=tcount, avg_hold=avg_hold, pct_days=pct_days, hit=hitrate, ann_return=ann_ret)

    is_mask = idx < IS_CUTOFF
    oos_mask = idx >= IS_CUTOFF

    return {
        "signal": signal, "position": pos, "strat_ret": strat_ret, "equity": curve, "costs": costs,
        "IS": seg(is_mask), "OOS": seg(oos_mask), "ALL": seg(pd.Series(True, index=idx))
    }

def main():
    p = argparse.ArgumentParser(description="HookCore signals builder (RSI3 cross + vol gate)")
    ...
    # Strategy params
    p.add_argument("--low", type=float, default=DEFAULT_LOW, help="RSI low threshold (default 35)")
    p.add_argument("--high", type=float, default=DEFAULT_HIGH, help="RSI high threshold (default 65)")
    p.add_argument("--volratio", type=float, default=DEFAULT_VOLRATIO, help="10d/60d vol must be < this (default 1.0)")
    p.add_argument("--cadence", choices=["biweekly","weekly","event"], default=DEFAULT_CADENCE)
    ...
    p.add_argument("--cost-bps", type=float, default=1.5, help="Per |Δposition|, in bps (default 1.5)")
    ...
    if args.date_col not in df_raw.columns or args.price_col not in df_raw.columns:
        ...
        df_raw = df_raw.rename(columns={df_raw.columns[0]:"Date", df_raw.columns[1]:"Price"})
    ...
    # Run
    res = run_strategy(df,
                       low_th=args.low, high_th=args.high, volratio_cap=args.volratio,
                       cadence=args.cadence, exec_mode=args.exec, hold_bars=args.hold,
                       vol_target=args.vol_target, vol_lb=args.vol_lb, lev_cap=args.lev_cap, cost_per_abs_delta=cost_per_abs_delta)

    # Output paths
    outdir = args.outdir or os.path.join("outputs", "hookcore", args.symbol, f"RSI3_{int(args.low)}_{int(args.high)}_vr{args.volratio}_{args.cadence}_{args.exec}_hold{args.hold}")
    ...
    pd.DataFrame({"equity": res["equity"]}, index=df.index).to_csv(os.path.join(outdir, "equity_curves.csv"))
