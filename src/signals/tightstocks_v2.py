#!/usr/bin/env python3
# src/cli/build_tightstocks_v2.py
"""
Build script for TightStocks v2

Continuous Inventory Investment Surprise (IIS) - Renaissance-style

Version 2 Changes:
- FIXED: Vol targeting classification from "sparse" to "always_on"
- Expected: ~10% realized vol (was 2.85% in v1)
- Expected: ~0.65-0.80 Sharpe (was 0.52 under-leveraged in v1)

Expected Performance (with proper vol targeting):
- Sharpe: 0.60-0.80 (conservative estimate)
- Realized vol: 10% ± 1%
- Correlation with price sleeves: ~0.06

Usage:
    python build_tightstocks_v2.py
...
from tightstocks_v1 import generate_tightstocks_v1_signal  # Same signal logic as v1
...
def main():
    ...
    # ========== 7. Print summary ==========
    print("=" * 80)
    print("TIGHTSTOCKS V2 - SUMMARY METRICS")
    print("=" * 80)
    ...
    print()
    print("V2 Changes:")
    print("  - Fixed vol targeting: strategy_type 'sparse' → 'always_on'")
    print(f"  - Target vol: 10.00%")
    print(f"  - Actual vol: {metrics['annual_vol']*100:.2f}%")
    vol_error = abs(metrics['annual_vol'] - 0.10) / 0.10 * 100
    if vol_error < 10:
        print(f"  - Vol targeting: ✓ Within 10% of target")
    else:
        print(f"  - Vol targeting: ⚠ {vol_error:.1f}% off target")
    print()
