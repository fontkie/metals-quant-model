...
def read_prices(con, prices_table: str) -> pd.DataFrame:
    df = pd.read_sql(f"SELECT * FROM {prices_table} ORDER BY date", con, parse_dates=["date"])
...
def read_signals(con, signals_table: str) -> pd.DataFrame:
    ...
    if "date" in df.columns:
        ...
        if "index" in df.columns:
            ...
            raise KeyError("signals table must contain a 'date' column or an 'index' column with dates.")
    # Keep the essentials only
    keep = [c for c in df.columns if c in ("trend_signal", "hook_signal")]
    if not keep:
        raise KeyError("signals table is missing 'trend_signal' and 'hook_signal'. Re-run build_signals.py.")
    return df[keep]
...
def ex_ante_leverage(returns: pd.Series, win=VOL_WIN, target_vol=TARGET_VOL, cap=LEVERAGE_CAP) -> pd.Series:
...
def metrics(pnl: pd.Series, turn: pd.Series, label: str, window: str) -> dict:
    ...
    n = len(pnl)
    if n == 0:
        return {"sleeve": label, "window": window, "ann_return": np.nan, "ann_vol": np.nan,
                "sharpe": np.nan, "sortino": np.nan, "max_dd": np.nan, "hit_rate": np.nan,
                "win_loss": np.nan, "avg_turnover": np.nan, "days": 0}
    eq = equity(pnl)
    cagr = eq.iloc[-1]**(ANNUAL_DAYS / n) - 1.0
    vol = pnl.std() * np.sqrt(ANNUAL_DAYS)
    sharpe = (pnl.mean() / pnl.std() * np.sqrt(ANNUAL_DAYS)) if pnl.std() > 0 else np.nan
    downside = pnl[pnl < 0]
    dstd = downside.std() * np.sqrt(ANNUAL_DAYS) if len(downside) > 0 else np.nan
    sortino = (pnl.mean() * np.sqrt(ANNUAL_DAYS) / dstd) if dstd and dstd > 0 else np.nan
    mdd = max_drawdown(eq)
    hit = (pnl > 0).mean()
    ...
    avg_turn = turn.mean()
    return {"sleeve": label, "window": window, "ann_return": cagr, "ann_vol": vol, "sharpe": sharpe,
            "sortino": sortino, "max_dd": mdd, "hit_rate": hit, "win_loss": wl, "avg_turnover": avg_turn, "days": n}
...
def main():
    ...
    ap.add_argument("--outdir", default=None, help="Optional base outputs dir (default: outputs/<Metal>)")
    args = ap.parse_args()
    ...
    # save equity CSV
    eq_df = pd.DataFrame({"eq_trend": eq_trend, "eq_hook": eq_hook, "eq_combo": eq_combo})
    ...
    for wname, (ws, we) in windows.items():
        ...
        rows.append(metrics(pnl_combo[mask], (0.5*turn_trend + 0.5*turn_hook)[mask], "combo50", wname))
