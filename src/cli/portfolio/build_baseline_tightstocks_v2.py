#!/usr/bin/env python3
"""
Build Baseline + TightStocks Portfolio v2 - IS/OOS VALIDATED
=============================================================
Renaissance-style methodology:
  1. IS (2011-2018): Grid search for optimal BL/TS weights
...
Usage:
  python build_baseline_tightstocks_v2.py --config path/to/config.yaml
"""
...
def blend_pnls(bl_pnl: pd.Series, ts_pnl: pd.Series, bl_weight: float) -> pd.Series:
    """Blend two PnL series"""
    ts_weight = 1.0 - bl_weight
    return bl_pnl.fillna(0) * bl_weight + ts_pnl.fillna(0) * ts_weight


def grid_search_weights(bl_pnl: pd.Series, ts_pnl: pd.Series, step: float = 0.05) -> tuple:
    """
    ...
    for bl_weight in weight_range:
        blended = blend_pnls(bl_pnl, ts_pnl, bl_weight)
        sharpe = calculate_sharpe(blended)
        
        all_results.append({
            'baseline_weight': bl_weight,
            'tightstocks_weight': 1.0 - bl_weight,
            'sharpe': sharpe
        })
...
def main():
    parser = argparse.ArgumentParser(description='Build Baseline + TightStocks v2 (IS/OOS validated)')
    parser.add_argument('--config', required=True, help='Path to config YAML')
    
    args = parser.parse_args()
    
    print("="*80)
    print("BASELINE + TIGHTSTOCKS v2 - IS/OOS VALIDATED")
    print("="*80)
    print()
    ...
    # Align dates
    common_dates = baseline.index.intersection(tightstocks.index)
    ...
    is_ts_pnl = ts_pnl.loc[is_dates]
    
    # OOS PnLs
    oos_bl_pnl = bl_pnl.loc[oos_dates]
    oos_ts_pnl = ts_pnl.loc[oos_dates]
    ...
    is_ts_sharpe = calculate_sharpe(is_ts_pnl)
    
    print(f"IS Standalone - Baseline: {is_bl_sharpe:.3f} Sharpe")
    print(f"IS Standalone - TightStocks: {is_ts_sharpe:.3f} Sharpe")
    print()
    
    # Grid search on IS
    best_bl_weight, is_best_sharpe, is_results = grid_search_weights(
        is_bl_pnl, is_ts_pnl, step=grid_step
    )
    best_ts_weight = 1.0 - best_bl_weight
    
    print(f"IS Optimal Weights:")
    print(f"  Baseline:    {best_bl_weight:.0%}")
    print(f"  TightStocks: {best_ts_weight:.0%}")
    ...
    # Correlation in IS
    is_correlation = is_bl_pnl.corr(is_ts_pnl)
    ...
    oos_ts_sharpe = calculate_sharpe(oos_ts_pnl)
    
    print(f"OOS Standalone - Baseline: {oos_bl_sharpe:.3f} Sharpe")
    print(f"OOS Standalone - TightStocks: {oos_ts_sharpe:.3f} Sharpe")
    print()
    
    # Apply frozen IS weights to OOS
    oos_blended = blend_pnls(oos_bl_pnl, oos_ts_pnl, best_bl_weight)
    ...
    # Correlation in OOS
    oos_correlation = oos_bl_pnl.corr(oos_ts_pnl)
    ...
    print(f"{'TightStocks Sharpe':<30} {is_ts_sharpe:>10.3f} {oos_ts_sharpe:>10.3f}")
    ...
    # TightStocks value-add check
    print()
    if oos_marginal > 0.05:
        print(f"✓ TightStocks adds significant value in OOS (+{oos_marginal:.3f} Sharpe)")
        ts_recommendation = "INCLUDE"
    elif oos_marginal > 0:
        print(f"~ TightStocks adds marginal value in OOS (+{oos_marginal:.3f} Sharpe)")
        ts_recommendation = "CONSIDER"
    else:
        print(f"✗ TightStocks does not add value in OOS ({oos_marginal:+.3f} Sharpe)")
        ts_recommendation = "EXCLUDE"
    ...
    daily_df = pd.DataFrame({
        ...
        'portfolio_pnl': blend_pnls(bl_pnl, ts_pnl, best_bl_weight).values,
        'baseline_weight': best_bl_weight,
        'tightstocks_weight': best_ts_weight,
    ...
    validation = {
        ...
        'optimal_weights': {
            ...
            'tightstocks': best_ts_weight
        },
        'is_metrics': {
            'baseline_sharpe': is_bl_sharpe,
            'tightstocks_sharpe': is_ts_sharpe,
        ...
        'oos_metrics': {
            ...
            'tightstocks_sharpe': oos_ts_sharpe,
        ...
        'validation': {
            ...
            'status': status,
            'tightstocks_recommendation': ts_recommendation
        }
    }
    ...
    print(f"  Weights: {best_bl_weight:.0%} BL / {best_ts_weight:.0%} TS")
    print(f"  TightStocks: {ts_recommendation}")
