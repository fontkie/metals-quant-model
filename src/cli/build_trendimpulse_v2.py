# src/cli/build_trendimpulse_v2.py
"""
TrendImpulse v2.0 Build Script (Layer B CLI Wrapper)
-----------------------------------------------------
...
from signals.trendimpulse import generate_trendimpulse_signal


def main():
    ap = argparse.ArgumentParser(
        description="Build TrendImpulse v2.0 (Copper) — canonical CSV + YAML"
    )
    ...
    print(f"[TrendImpulse] Loading canonical CSV: {args.csv}")
    ...
    print(
        f"[TrendImpulse] Loaded {len(df)} rows from {df['date'].min()} to {df['date'].max()}"
    )

    # ========== 2. LOAD YAML CONFIG ==========
    print(f"[TrendImpulse] Loading config: {args.config}")
    ...
    print(f"[TrendImpulse] Config validated ✓")

    # ========== 3. CALCULATE RETURNS (NEEDED FOR LAYER A) ==========
    df["ret"] = df["price"].pct_change().fillna(0.0)

    # ========== 4. GENERATE SIGNAL ==========
    print(f"[TrendImpulse] Generating signal...")

    mom_cfg = signal_cfg["momentum"]
    trend_cfg = signal_cfg["trend_filter"]

    df["pos_raw"] = generate_trendimpulse_signal(
        df=df,
        roc_lookback=mom_cfg["roc_lookback"],
        zscore_lookback=mom_cfg["zscore_lookback"],
        entry_threshold=mom_cfg["entry_threshold"],
        exit_threshold=mom_cfg["exit_threshold"],
        trend_filter_enabled=trend_cfg["enabled"],
        trend_ma_lookback=trend_cfg["ma_lookback"],
        ma_shift=mom_cfg.get("shift_bars", 1),
    )

    # ========== 5. RUN LAYER A CORE ==========
    print(f"[TrendImpulse] Running Layer A execution contract...")
    ...
    print(f"[TrendImpulse] Wrote daily_series.csv → {daily_series_path}")

    # Write metrics
    metrics_path = outdir / "summary_metrics.json"
    with open(metrics_path, "w") as f:
        json.dump(metrics, f, indent=2)
    print(f"[TrendImpulse] Wrote summary_metrics.json → {metrics_path}")

    # ========== 7. PRINT SUMMARY ==========
    print("\n" + "=" * 60)
    print("TrendImpulse v2.0 Build Complete")
    print("=" * 60)
